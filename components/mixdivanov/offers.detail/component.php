...
$arDesignReplace = [];
// Если страница готового дизайна
if (isset($_REQUEST['IS_DESIGN']) && $_REQUEST['IS_DESIGN'] == 'Y')
{
	$arSelectDesigns = Array('IBLOCK_ID', 'ID', 'NAME', 'CODE', 'DETAIL_PICTURE', 'PREVIEW_PICTURE', 'PROPERTY_OFFER', 'PROPERTY_PHOTO_CLOTH_GROUP');
	$arFilterDesigns = Array("IBLOCK_ID"=>6, "ACTIVE"=>"Y", 'CODE' => $arParams["ELEMENT_CODE"]);
	$resDesigns = CIBlockElement::GetList(['SORT' => 'ASC', 'DATE_CREATE' => 'DESC'], $arFilterDesigns, false, false, $arSelectDesigns);
	
	if($arDesign = $resDesigns->GetNext())
	{
		$arDesignReplace = $arDesign;
		$arParams["~ELEMENT_ID"] = $arDesign['PROPERTY_OFFER_VALUE'];
		$arParams["ELEMENT_CODE"] = false;
	}
...
if($arParams["SHOW_WORKFLOW"] || $this->startResultCache(false, array(($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()),$bUSER_HAVE_ACCESS, $arNavigation, $pagerParameters)))
{
	...
	if ($obElement = $rsElement->GetNextElement()) {
		...
		// Группа ткани по умолчанию
		$arResult['DEFAULT_CLOTH_GROUP'] = 1;
		$arResult['IS_DESIGN'] = false;
		// Готовые дизайны
		$arResult['DESIGNS'] = [];

		// Подмена на данные из готового дизайна
		if (count($arDesignReplace > 0) && isset($arDesignReplace['ID'])) {
			$arResult['IS_DESIGN'] = true;
			$arResult["DETAIL_PICTURE"]["SRC"] = CFile::GetPath($arDesignReplace['DETAIL_PICTURE']);
			$arResult['DEFAULT_CLOTH_GROUP'] = $arDesignReplace['PROPERTY_PHOTO_CLOTH_GROUP_VALUE'];
		}

		$arSelectDesigns = ['IBLOCK_ID', 'ID', 'NAME', 'CODE', 'DETAIL_PICTURE', 'PREVIEW_PICTURE', 'PROPERTY_OFFER', 'PROPERTY_PHOTO_CLOTH_GROUP'];
		$arFilterDesigns = ["IBLOCK_ID"=>6, "ACTIVE"=>"Y", 'PROPERTY_OFFER' => $arResult['ID']];
		$resDesigns = CIBlockElement::GetList(['SORT' => 'ASC', 'DATE_CREATE' => 'DESC'], $arFilterDesigns, false, false, $arSelectDesigns);
		
		while($arDesign = $resDesigns->GetNext()) {
 			$arResult['DESIGNS'][] = $arDesign;
		}

		// Механизм трансформации
		$arResult['MECHANISM'] = CTransformMechanism::GetByCode($arResult['PROPERTIES']['MECHANISM']['VALUE']);

		// Производитель
		$arResult['MANUFACTURER'] = CManufacturer::Get($arResult['PROPERTIES']['MANUFACTURER']['VALUE']);
		$arResult['CHARACTERISTICS'] = CCharacteristics::GetByCode($arResult['PROPERTIES']['CHARACTERISTICS']['VALUE']);
		$arResult['CATALOG_PRODUCT'] = CCatalogProduct::GetByID($arResult['ID']);
		$arResult['BASE_PRICE'] = CPrice::GetBasePrice($arResult['ID']);
		$arResult['BASE_PRICE_VALUE'] = intval($arResult['BASE_PRICE']["PRICE"]);

		$arTable = [];
		$priceTable = $arResult['PROPERTIES']['PRICE_TABLE']['~VALUE'];

		if (strlen($priceTable) > 1) {
			$arTable = json_decode($priceTable, true);
		}
		...
		foreach ($arTable as $idRow => $arRow) {
		    ...
		    foreach ($arRow['groups'] as $groupN => $groupMarkup) {
		    	$groupPrice = $arResult['BASE_PRICE_VALUE'] + $groupMarkup;
		    	CManufacturer::ApplyMarkup($arResult['MANUFACTURER'], $groupPrice);
		    	CManufacturer::ApplyMarkupStep($arResult['MANUFACTURER'], $groupPrice, $groupN);
				CUtils::RoundPrice($groupPrice);

				$arTable[$idRow]['groups'][$groupN] = $groupPrice;
		    }
		}

		$arResult['PROPERTIES']['PRICE_TABLE']['~VALUE'] = json_encode($arTable);
		$arResult['PRICE_TABLE'] = $arTable;

		// Группы тканей (1,2,3,4,...)
		$arResult['CLOTHES']['GROUPS'] = Clothes::GetGroups();
		// Типы тканей (жаккард, велюр)
		$arResult['CLOTHES']['TYPES'] = CClothesTypes::GetAll();

		// Насильно добавим опции для третьей
		if (array_search('third_cloth', $arResult['PROPERTIES']['AVAILABLE_OPTIONS']['VALUE']) === false) {
			$arResult['PROPERTIES']['AVAILABLE_OPTIONS']['VALUE'][] = 'third_cloth';
		}

		// Насильно добавим опции для четвертой ткани
		if (array_search('fourth_cloth', $arResult['PROPERTIES']['AVAILABLE_OPTIONS']['VALUE']) === false) {
			$arResult['PROPERTIES']['AVAILABLE_OPTIONS']['VALUE'][] = 'fourth_cloth';
		}

		// Доступные доп.опции
		$arResult['OPTIONS'] = CFurnitureOptions::GetByCode($arResult['PROPERTIES']['AVAILABLE_OPTIONS']['VALUE']);
		// Самая обычная/минимальная цена
		$arResult['START_PRICE'] = $arTable[0]['groups'][$arResult['DEFAULT_CLOTH_GROUP']];
		// Массив отзывов
		$arResult['REVIEWS'] = CReview::GetList($arResult['ID'], 2);
		// Массив сертификатов
		$arResult['CERTIFICATES'] = CCertificates::GetList();
		// 
		if (in_array('WOODEN_COVERS', $arResult['PROPERTIES']['FILTER_CHARACTERISTICS']['VALUE'])) {
			$arResult['ARMRESTS_COLORS'] = CArmrestsColors::GetAll();
		}

		$arResult["IBLOCK"] = GetIBlock($arResult["IBLOCK_ID"], $arResult["IBLOCK_TYPE"]);

		$ipropTemplates = new Iblock\InheritedProperty\ElementTemplates($arResult["IBLOCK_ID"], $arResult["ID"]);
		$foundTemplates = $ipropTemplates->findTemplates();
		
		/** Подмена значения у свойства PRICE_TABLE**/
		foreach ($foundTemplates as $templateKey => $arElementTemplate) {
			
			if (strpos($arElementTemplate["TEMPLATE"], '{=this.property.PRICE_TABLE}') !== false) {
				$arElementTemplate["TEMPLATE"] = str_replace('{=this.property.PRICE_TABLE}', $arResult['START_PRICE'], $arElementTemplate["TEMPLATE"]);

				$ipropTemplates->set([
					$templateKey => $arElementTemplate["TEMPLATE"]
				]);
			}
		}

		// Добавпим к имени префикс от раздела (UF_OFFER_PREFIX)
		$arResult['NAME'] = trim(OffersNaming::getSectionPrefix($arResult['IBLOCK_SECTION_ID']) . ' ' . $arResult['NAME']);

		// Заново получим шаблоны элемента
		$arResult["IPROPERTY_VALUES"] = $ipropValues->getValues();
		/****/
